...
classParameter
    : modifierList? (VAL | VAR)? simpleIdentifier COLON ktype (ASSIGNMENT expression)?
    ;
...
getter
    : modifierList? GETTER
    | modifierList? GETTER NL* LPAREN RPAREN (NL* COLON NL* ktype)? NL* (block | ASSIGNMENT NL* expression)
    ;
...
typeAlias
    : modifierList? TYPE_ALIAS NL* simpleIdentifier (NL* typeParameters)? NL* ASSIGNMENT NL* ktype
    ;
...
typeParameter
    : modifierList? NL* simpleIdentifier (NL* COLON NL* ktype)?
    ;
...
functionType
    : (functionTypeReceiver NL* DOT NL*)? functionTypeParameters  NL* ARROW (NL* ktype)
    ;
...
functionTypeParameters
    : LPAREN (parameter | ktype)? (COMMA (parameter | ktype))* RPAREN
    ;
...
typeConstraint
    : annotations* simpleIdentifier NL* COLON NL* ktype
    ;
...
namedInfix
    : elvisExpression ((inOperator NL* elvisExpression)+ | (isOperator NL* ktype))?
    ;
...
atomicExpression
    ...
    | superExpression // SUPER (LANGLE ktype RANGLE)? labelReference?
...
typeProjection
    : typeProjectionModifierList? ktype | MULT
    ;
...
lambdaParameter
    : variableDeclaration
    | multiVariableDeclaration (NL* COLON NL* ktype)?
    ;
...
superExpression
    : SUPER (LANGLE NL* ktype NL* RANGLE)? LabelReference?
    ;
